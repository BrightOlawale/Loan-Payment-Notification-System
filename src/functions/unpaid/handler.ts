...
const handler: Handler = async (event) => {
  try {
    ...
    await loanService.sendUnpaidNotification(unpaidData);

    return {
      statusCode: 200,
      body: JSON.stringify({ message: "Loan unpaid event processed" }),
    };
  } catch (error) {
    logger.error("Loan unpaid event processing failed");
    throw new Error("Loan unpaid event processing failed");
  }
};
